...
Začeli bomo z analizo voženj in postaj ter pogledali vpliv Covida-19 in vremena na trende.
...
Graf ponovno pokaže, da je delitev glede na tip naročnine zelo pomembna, saj se različna tipa uporabnikov obnašata različno. Člani opravijo največ voženj ravno v času jutranje in popodanske gužve, ko so ljudje na poti v službo. Nečlani pa so bolj aktivni popoldne in zvečer, v času socialnih aktivnosti. Zdaj lahko bolje utrdimo, da je Covid-19 močneje vplival na člane, saj so morda delali od doma.

```{r}
print(graf9)
```

Opazili smo že, da je število voženj močno povezano z letnim časom, ki je seveda povezan z povprečno temperaturo. Jasno je, da se bo več voženj opravilo pri višjih temperaturah.

```{r}
print(graf10)
```

Škatle z brki nam pričakovano pokažejo, da poleg temperature tudi ostale vremenske razmere vplivajo na število prevozov. V splošnem lepše kot je vreme, več izposoj je opravljenih.
